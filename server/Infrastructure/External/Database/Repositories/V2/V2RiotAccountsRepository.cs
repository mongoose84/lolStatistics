...
public class V2RiotAccountsRepository : RepositoryBase
{
    ...
    public Task UpdateSyncStatusAsync(string puuid, string syncStatus, DateTime? lastSyncAt = null)
    {
        const string sql = @"UPDATE riot_accounts
            SET sync_status = @sync_status, last_sync_at = @last_sync_at, updated_at = @updated_at
            WHERE puuid = @puuid";
        return ExecuteNonQueryAsync(sql,
            ("@puuid", puuid),
            ("@sync_status", syncStatus),
            ("@last_sync_at", lastSyncAt ?? DateTime.UtcNow),
            ("@updated_at", DateTime.UtcNow));
    }
    ...
    public Task SetPrimaryAsync(string puuid, long userId)
    {
        ...
        return ExecuteTransactionAsync(async conn =>
        {
            await ExecuteNonQueryWithConnectionAsync(conn, unsql, ("@user_id", userId), ("@updated_at", DateTime.UtcNow));
            await ExecuteNonQueryWithConnectionAsync(conn, setsql, ("@puuid", puuid), ("@user_id", userId), ("@updated_at", DateTime.UtcNow));
        });
    }
